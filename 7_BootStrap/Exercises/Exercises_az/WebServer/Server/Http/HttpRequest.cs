...
namespace WebServer.Server.Http
{
    ...
    public class HttpRequest : IHttpRequest
    {
        ...
        public HttpRequest(string requestText)
        {
            ...
            this.Cookies = new HttpCookieCollection();  //4 COOCIES
            this.Session = new HttpSession(SessionStore.SessionCookieKey);
        ...
        public IHttpCookieCollection Cookies { get; set; }  //3 COOCIES
        ...
        public IHttpSession Session { get; set; } //3.1 Session
        ...
        private void ParseRequest(string requestText)
        {
            ...
            this.ParseCookies();           //1 COOCIES    
            this.ParseParameters();
            this.ParseFormData(requestLines.Last());

            this.SetSession();  //3.3Session
        }
        ...
        private void ParseCookies()               //2 COOCIES
        {
            if (this.Headers.ContainsKey(HttpHeader.Cookie))
            {
                var allCookies = this.Headers.Get(HttpHeader.Cookie);
                //Cookie: lang=en; -> key = Cookie,value = lang=en;domain=...;...
                foreach (var x in allCookies)
                {
                    if (!x.Value.Contains('='))
                    {
                        return;
                    ...
                    foreach (var y in cookieParts)
                    {
                        var cookieKeyValuePair = y
                            .Split(new[] { '=' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        private void SetSession()//3.4 Session
        {
            
            //browser send Cookis-> Cookie: SID=strhyhy52rtyh2eytt55r SID-Session id
            if (this.Cookies.ContainsKey(SessionStore.SessionCookieKey))
            {
                ...
                //if sessionId exist will send back, when not exist will be created
            }
        }
