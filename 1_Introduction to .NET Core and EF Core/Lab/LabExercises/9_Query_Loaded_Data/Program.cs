    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new ShopDbContext())
            {
                ...
                SaveItems(db);
                ProcessComandDb(db);
                PrintOrdersWhitMoreThenoneItem(db);
            }
        }
        ...
        private static void PrintOrdersWhitMoreThenoneItem(ShopDbContext db)
        {
            var customerId = int.Parse(Console.ReadLine());

            var result = db.Orders
                .Where(c => c.CustomerId == customerId)
                .Where(c => c.ItemOrders.Count > 1)
                .Select(c => new
                {
                    OrdersCount = c.ItemOrders.Count,                   
                })
                .FirstOrDefault();

            Console.WriteLine($"Orders count:{result.OrdersCount}");            
        }
